...
public class WavelengthServer implements DedicatedServerModInitializer {
    ...
    public void onInitializeServer() {
        ...
        net.fabricmc.fabric.api.entity.event.v1.ServerEntityWorldChangeEvents
                .AFTER_PLAYER_CHANGE_WORLD.register((player, origin, destination) -> {
                    boolean netherBlocked = destination.getRegistryKey().equals(net.minecraft.world.World.NETHER)
                            && !DimensionGateManager.isNetherOpen();
                    boolean endBlocked    = destination.getRegistryKey().equals(net.minecraft.world.World.END)
                            && !DimensionGateManager.isEndOpen();
                    if (!netherBlocked && !endBlocked) return;
                    String dimName = netherBlocked ? "Nether" : "End";
                    player.sendMessage(Text.literal("§c⚠ The " + dimName + " is currently closed by the server."), true);

                    net.minecraft.server.MinecraftServer srv = destination.getServer();
                    if (srv == null) return;
                    net.minecraft.server.world.ServerWorld overworld = srv.getWorld(net.minecraft.world.World.OVERWORLD);
                    if (overworld == null) return;

                    final double rx = 0.5, rz = 0.5;
                    final double ry = overworld.getTopY(net.minecraft.world.Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, 0, 0);
                    srv.execute(() -> player.teleport(overworld, rx, ry, rz,
        ...
        ServerPlayNetworking.registerGlobalReceiver(
                ...
                (payload, context) -> {
                    ...
                    if (!isOp) return;
                    DimensionGateManager.apply(payload.dimension(), payload.enable(), payload.linkedEventId());
                    broadcastDimensionGateSync(context.server());
                }
        );
        ...
        ServerPlayNetworking.registerGlobalReceiver(
                ...
                (payload, context) -> {
                    ...
                    if (payload.isCreate()) {
                        ...
                        boolean isInstant = payload.endMs() == -1L;
                        boolean hasDelay  = isInstant && payload.scheduledStartMs() > System.currentTimeMillis();
                        long    effectiveEndMs = isInstant
                                ? (hasDelay ? payload.scheduledStartMs() + 1000L : 0L)
                                : payload.endMs();

                        event = ServerEventData.create(senderUuid, senderName,
                                payload.name(), payload.description(),
                                payload.hasLocation(), payload.locX(), payload.locY(), payload.locZ(),
                                effectiveEndMs, payload.scheduledStartMs());
                        if (event == null) return;

                        if (isInstant && !hasDelay) {

                            DimensionGateManager.commitEventStart(event.id);
                            ServerEventData.end(event.id, senderUuid);
                            DimensionGateManager.onEventEnded(event.id, true);
                            broadcastDimensionGateSync(server);
                ...
                (payload, context) -> {
                    ...
                    switch (payload.action()) {
                        case "END" -> {
                            ...
                            boolean wasRunning = ev != null && ev.active && !ev.isScheduled();
                            ...
                            if (ok) {
                                ...
                                DimensionGateManager.onEventEnded(payload.eventId(), wasRunning);
                                broadcastDimensionGateSync(server);
                            }
                        }
                        case "DELETE" -> {
        ...
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            ...
            ServerPlayNetworking.send(newPlayer, buildDimensionGateSyncPayload(server));
        ...
        ServerTickEvents.END_SERVER_TICK.register(server -> {
            ...
            {
                String nLink = DimensionGateManager.getNetherLinkedEventId();
                String eLink = DimensionGateManager.getEndLinkedEventId();
                if (!nLink.isEmpty() || !eLink.isEmpty()) {
                    boolean gateChanged = false;
                    if (!nLink.isEmpty()) {
                        EventData nEv = ServerEventData.get(nLink);
                        if (nEv != null && nEv.active && !nEv.isScheduled()) {
                            DimensionGateManager.commitEventStart(nLink);
                            gateChanged = true;
                        }
                    }
                    if (!eLink.isEmpty()) {
                        EventData eEv = ServerEventData.get(eLink);
                        if (eEv != null && eEv.active && !eEv.isScheduled()) {
                            DimensionGateManager.commitEventStart(eLink);
                            gateChanged = true;
                        }
                    }
                    if (gateChanged) broadcastDimensionGateSync(server);
                }
            }
            ...
            for (EventData ev : ServerEventData.getActive()) {
                if (ev.endMs > 0 && ev.endMs <= now) {
                    ...
                    boolean wasRunning = !ev.isScheduled();
                    ...
                    if (ended) {
                        ...
                        DimensionGateManager.onEventEnded(ev.id, wasRunning);
                        broadcastDimensionGateSync(server);
        ...
        net.fabricmc.fabric.api.event.lifecycle.v1.ServerLifecycleEvents.SERVER_STOPPING.register(stoppingServer -> {
            ServerProjectData.saveNow();
            ServerEventData.saveNow();
            DimensionGateManager.saveNow();
            Wavelength.LOGGER.info("[Wavelength] Flushed all server data on shutdown.");
        });
        ...
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) ->
                dispatcher.register(
                        CommandManager.literal("wldebug")
                                .requires(src -> {
                                    try {
                                        PermissionPredicate p = src.getPlayerOrThrow().getPermissions();
                                        return p == LeveledPermissionPredicate.GAMEMASTERS
                                                || p == LeveledPermissionPredicate.ADMINS
                                                || p == LeveledPermissionPredicate.OWNERS;
    ...
    static void broadcastDimensionGateSync(MinecraftServer server) {
        SyncPacket.DimensionGateSyncPayload payload = buildDimensionGateSyncPayload(server);
        server.getPlayerManager().getPlayerList()
                .forEach(p -> ServerPlayNetworking.send(p, payload));
    }

    private static SyncPacket.DimensionGateSyncPayload buildDimensionGateSyncPayload(MinecraftServer server) {
        String nEId = DimensionGateManager.getNetherLinkedEventId();
        String eEId = DimensionGateManager.getEndLinkedEventId();
        EventData nEv = nEId.isEmpty() ? null : ServerEventData.get(nEId);
        EventData eEv = eEId.isEmpty() ? null : ServerEventData.get(eEId);
        return new SyncPacket.DimensionGateSyncPayload(
                DimensionGateManager.isNetherOpen(),        DimensionGateManager.isEndOpen(),
                DimensionGateManager.getNetherBaseEnabled(), DimensionGateManager.getEndBaseEnabled(),
                nEId, eEId,
                nEv != null ? nEv.name : "",
                eEv != null ? eEv.name : "");
    }
