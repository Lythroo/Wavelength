...
public final class SyncPacket {
    ...
    public static final CustomPayload.Id<DimensionGateSyncPayload> DIMENSION_GATE_SYNC_ID =
            new CustomPayload.Id<>(Identifier.of(Wavelength.MOD_ID, "dimension_gate_sync"));

    public record DimensionGateSyncPayload(
            boolean netherOpen,       boolean endOpen,
            boolean netherBase,       boolean endBase,
            String  netherEventId,    String  endEventId,
            String  netherEventName,  String  endEventName
    ) implements CustomPayload {
        public static final PacketCodec<PacketByteBuf, DimensionGateSyncPayload> CODEC =
                new PacketCodec<>() {
                    @Override
                    public DimensionGateSyncPayload decode(PacketByteBuf buf) {
                        return new DimensionGateSyncPayload(
                    ...
                    @Override
                    public void encode(PacketByteBuf buf, DimensionGateSyncPayload p) {
                        buf.writeBoolean(p.netherOpen());   buf.writeBoolean(p.endOpen());
                        buf.writeBoolean(p.netherBase());   buf.writeBoolean(p.endBase());
                        buf.writeString(p.netherEventId()); buf.writeString(p.endEventId());
                        buf.writeString(p.netherEventName()); buf.writeString(p.endEventName());
                    }
                };
        @Override public Id<? extends CustomPayload> getId() { return DIMENSION_GATE_SYNC_ID; }
    }

    public static final CustomPayload.Id<DimensionGateActionC2SPayload> DIMENSION_GATE_ACTION_C2S_ID =
            new CustomPayload.Id<>(Identifier.of(Wavelength.MOD_ID, "dimension_gate_action"));

    public record DimensionGateActionC2SPayload(
            String dimension, boolean enable, String linkedEventId
    ) implements CustomPayload {
        public static final PacketCodec<PacketByteBuf, DimensionGateActionC2SPayload> CODEC =
                PacketCodec.tuple(
                        PacketCodecs.STRING,  DimensionGateActionC2SPayload::dimension,
                        PacketCodecs.BOOLEAN, DimensionGateActionC2SPayload::enable,
                        PacketCodecs.STRING,  DimensionGateActionC2SPayload::linkedEventId,
                        DimensionGateActionC2SPayload::new);
        @Override public Id<? extends CustomPayload> getId() { return DIMENSION_GATE_ACTION_C2S_ID; }
    }}